...
function twitter_fetch_items($keyword,$camp ){
	...
	if(stristr($exec, 'search_metadata')   || (stristr($keyword, 'from:') && stristr($exec, '{')  ) ){
		...
		foreach ($items as $item){
			...
			//original post link
			$original_post_url =  '';
			$original_post_url = $item->entities->urls[0]->expanded_url;
			...
			if(isset($item->entities->media[0])){
				...
				if($media_img->type == 'photo'){
					...
					$itm['item_image'] = $media_img->media_url;
				}
		...
		if($i == 0 || ($i==1 && $max_id == $wp_tw_next_max_id) ){
			...
			if($max_id != 0){
				...
				update_post_meta($camp->camp_id, 'wp_twitter_next_max_id'.md5($keyword), $max_id ) ;

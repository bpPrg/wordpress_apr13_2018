	...
	foreach ( $keywords as $keyword ) {
		...
		if (trim ( $keyword ) != '') {
			...
			if (count ( $res ) > 0) {
				...
				}elseif( stristr($camp->camp_post_content, 'item_user_username') || stristr($camp->camp_post_content, 'item_user_name') || stristr($camp->camp_post_content, 'item_user_profile_pic') || stristr($camp->camp_post_title, 'item_user_username') || stristr($camp->camp_post_title, 'item_user_name') || stristr($camp->camp_post_title, 'item_user_profile_pic') || in_array('OPT_CUSTOM', $camp_opt) ){
				...
				if($isDetailedItemInfoRequired){
					...
					if(in_array('OPT_IT_SLIDER', $camp_opt)){
						...
						if(isset($fullItemDetails->graphql->shortcode_media->edge_sidecar_to_children->edges)){
							...
							if(trim($img_template) == '') $img_template = '<img src="[img_src]" class="wp_automatic_gallery" />';
					...
					if( isset($fullItemDetails->graphql->shortcode_media->shortcode	) && trim($fullItemDetails->graphql->shortcode_media->shortcode) == $t_data['item_id'] ){
						...
						if(isset($temp['is_video']) && $temp['is_video'] == 'yes' ){
							...
							$videoEmbed = ' [video src="' . $fullItemDetails->graphql->shortcode_media->video_url .'" '.$autoPlay.'] ' ;
						...
						if(in_array('OPT_IT_COMMENT', $camp_opt)){
							...
							$commentsArray = $fullItemDetails->graphql->shortcode_media->edge_media_to_comment->edges;
...
function instagram_fetch_items($keyword,$camp ){
	...
	if( isset($jsonArr->status) ){
		...
		if( in_array ( 'OPT_IT_USER' ,  $camp_opt ) ){
			...
			$page_info = $jsonArr->data->hashtag->edge_hashtag_to_top_posts->page_info;

	...
	*/
	...
	if (! $this->is_duplicate($url) ) {
		...
		// Class or ID extraction
			...
			if(1){
				...
				foreach ($cg_feed_custom_id as $cg_selecotr_data){
					...
					$res['rule_'.$rule_num.'_plain'] = strip_tags( $content );
		...
		if(in_array('OPT_FEED_LAZY',$camp_opt)){
			...
			preg_match_all('{<img .*?>}', $res['cont'],$imgsMatchs);
		...
		if(stristr($res['cont'], 'twitter.com') && ! stristr($res['cont'], 'platform.twitter') ){
			...
			$res['cont'].= '<script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>';
		}
		...
		if(stristr($res['cont'], 'instagram.com') && ! stristr($res['cont'], 'platform.instagram') ){
			...
			$res['cont'].= '<script async defer src="//platform.instagram.com/en_US/embeds.js"></script>';
		}
		...
		if( in_array ( 'OPT_MUST_IMAGE', $camp_opt ) &&   ! stristr($res['cont'], '<img') && trim($currentOgImage) == '' ) {
			...
			if(! in_array('OPT_FEED_SRCSET', $camp_opt)){
				$res['cont'] = preg_replace('{srcset=".*?"}', '', $res['cont']);
				$res['cont'] = preg_replace('{sizes=".*?"}', '', $res['cont']);

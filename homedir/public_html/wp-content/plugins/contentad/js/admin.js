jQuery(document).ready(function($) {
	...
	var headingPosition = '.wrap ' + ( $('h1').length ) ? 'h1' : 'h2' ; // get first tag name inside .wrap div
	...
	$('.row-actions .pause a').live('click', function(e){
		...
		$.post(
			...
			function( response ){
				...
				activeColumn.html( '<span class="contentad-inactive-state">Paused</span>' );
			},
			'json'
		);
	});
	...
	$('.row-actions .activate a').live('click', function(e){
		...
		$.post(
			...
			function( response ){
				...
				link.text( text ).attr( 'title', text).closest('span').attr( 'class', text.toLowerCase() );
				activeColumn.html( '<span class="contentad-active-state">Active</span>' );
			},
			'json'
		);

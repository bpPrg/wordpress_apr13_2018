...
class Wphigh_Theme_Framework {	
	...
	public function setup() {
		 ...
		 */
		load_theme_textdomain( 'radar', get_template_directory() . '/languages' );
		...
		add_image_size( 'radar-full-width', 1140, 530, true );
	
		// This theme uses wp_nav_menu() in two locations.
		register_nav_menus( array(
			'primary'   => __( 'Top primary menu', 'radar' )
		) );
	...
	public function widgets_init() {
		register_sidebar( array(
			'name'          => __( 'Sidebar', 'radar' ),
			'id'            => 'sidebar',
			'description'   => __( 'Additional sidebar that appears on the right.', 'radar' ),
...
<span class="sr-only">30% Complete</span></div></div>',
		...
		register_sidebar( array(
			'name'          => __( 'Footer Widget Area', 'radar' ),
			'id'            => 'footer',
			'description'   => __( 'Appears in the footer section of the site.', 'radar' ),
	...
	public function enqueue_scripts() {
		// Google font: Oswald
		wp_enqueue_style( 'radar-google-font-oswald', 'http://fonts.googleapis.com/css?family=Oswald' );
		
		// Load style.css
		wp_enqueue_style( 'radar-style', get_stylesheet_uri(), array( 'radar-bootstrap' ) );
		
		// Load theme main js.
		wp_enqueue_script( 'radar-main', get_template_directory_uri() . '/assets/js/main.js', array(
			'jquery',
			'radar-bootstrap'
		), '', true );
	...
	public function wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 ) {
			$title = "$title $sep " . sprintf( __( 'Page %s', 'radar' ), max( $paged, $page ) );
		}
	...
	public function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'radar_copyright', array(
			'title'    => __('Copyright', 'radar'),
			'description' => __( 'Enter the copyright text is displayed in the bottom left corner of the page.', 'epb' ),
			'priority' => 140,
		));
		
		$wp_customize->add_setting( 'radar_copyright', array(
			'capability'     		=> 'edit_theme_options',
			'type'           		=> 'option',
			'sanitize_callback'		=> 'wp_kses_post'
		));
		
		$wp_customize->add_control( 'radar_copyright_text', array(
			'label'      => '',
			'section'    => 'radar_copyright',
			'settings'   => 'radar_copyright',
		));				
	}

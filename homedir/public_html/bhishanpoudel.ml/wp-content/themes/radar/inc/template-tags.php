...
function radar_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'radar_category_count' ) ) ) {
		...
		set_transient( 'radar_category_count', $all_the_cool_cats );
	}
...
function radar_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'radar_category_count' );
}
...
function radar_post_footer() {	
	...
	?>
	<footer class="entry-meta btn-group btn-group-justified"<?php echo $style; ?>>
	<?php
		if ( radar_is_sticky() )
			$option = 'danger';
		else
...
function radar_post_meta( $option = '', $size = '' ) {	
	...
	if ( radar_is_sticky() ) {
		...
		$button = $radar_bs->get_button( $icon, array(
			...
			'title' => esc_attr( __( 'Sticky', 'radar' ) ),
		) );
	...
	if ( ! radar_is_sticky() ) {
		...
		$button = $radar_bs->get_button( $icon, array(
			'class' => "$btn_size $btn_option radar-tooltip",
			'title' => esc_attr( get_the_date() . ' @ ' . get_the_time() )
		) );
	...
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		$button = $radar_bs->get_button( $icon . '&nbsp;&nbsp;' . get_comments_number(), array(
			'class' => "$btn_size $btn_option radar-tooltip",
	...
	if ( is_attachment() && $post->post_parent ) {		
		...
		$button = $radar_bs->get_button( $icon, array(
			'class' => "$btn_size $btn_option radar-tooltip",
			'title' => sprintf( __( 'Published In %s' , 'radar' ), get_the_title( $post->post_parent ) ),
...
function radar_paging_nav() {
	...
	$links = paginate_links( array(
		...
		'prev_text' => __( '&laquo;', 'radar' ),
		'next_text' => __( '&raquo;', 'radar' ),
		'type'      => 'array'
	) );
	...
	?>
		<nav class="navigation paging-navigation" role="navigation">
			<h3 class="sr-only"><?php _e( 'Posts navigation', 'radar' ); ?></h3>
			<ul class="pagination">
			<?php 
...
function radar_link_pages() {	
	wp_link_pages( array(
		'before'      => '<hr><div class="page-links text-center"><span class="page-links-title sr-only">' . __( 'Pages:', 'radar' ) . '</span><ul class="pagination pagination-sm">',
		'after'       => '</ul></div>',
		'link_before' => '<span>',
		'link_after'  => '</span>',
	) );
...
function radar_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h3 class="sr-only"><?php _e( 'Post navigation', 'radar' ); ?></h3>
...
function radar_archive_header() {
	...
	// Search
	if ( is_search() ) {
		$title = sprintf( __( 'Search Results for: %s', 'radar' ), get_search_query() );
			
	// Author
	} elseif ( is_author() ) {
		the_post();
		$title = sprintf( __( 'All posts by %s', 'radar' ), get_the_author() );
	...
	// Date	
		...
		if ( is_day() ) {
			$title = sprintf( __( 'Daily Archives: %s', 'radar' ), get_the_date() );

		} elseif ( is_month() ) {
			$title = sprintf( __( 'Monthly Archives: %s', 'radar' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'radar' ) ) );

		} elseif ( is_year() ) {
			$title = sprintf( __( 'Yearly Archives: %s', 'radar' ), get_the_date( _x( 'Y', 'yearly archives date format', 'radar' ) ) );

		} else {
			$title = __( 'Archives', 'radar' );	
		}
	...
	// Taxonomy	
		...
		$title = single_term_title( $tax_name . __( ' Archives: ', 'radar' ), false );
...
function radar_the_attached_image() {
	...
	if ( count( $attachment_ids ) > 1 ) {
		...
		printf( '<a href="%1$s" rel="attachment">%2$s</a>',
			...
			wp_get_attachment_image( $post->ID, 'radar-full-width' )
		);		
	}
	
	// Or Show image attachment img tag only
	else {
		echo wp_get_attachment_image( $post->ID, 'radar-full-width' );
	}

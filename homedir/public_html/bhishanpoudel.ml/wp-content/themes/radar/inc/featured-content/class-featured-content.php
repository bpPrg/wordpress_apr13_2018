...
class Wphigh_Featured_Content {
	...
	public static function add_featured_content_support() {
		add_theme_support( 'featured-content', array(
			'featured_content_filter' => 'radar_get_featured_posts',
			'max_posts'               => 6,
		) );		
	}	
	...
	public static function customize_register( $wp_customize ) {		
		...
		if ( empty( $tag_name ) ) {
			$tag_link = _x( 'tag', 'featured content description' , 'radar' );
		} else {
			$tag_link = sprintf( '<a href="%1$s">"%2$s" %3$s</a>', admin_url( '/edit.php?tag=' . $tag_name ), $tag_name, _x( 'tag', 'featured content description' , 'radar' ) );
		}
		
		$wp_customize->add_section( 'featured_content', array(
			'title'          => __( 'Featured Content', 'radar' ),
			'description'    => sprintf( __( 'Use the %s to feature your posts. You can change this to a tag of your choice; if no posts match the tag, Nothing will be displayed.', 'radar' ), $tag_link ),
			'priority'       => 130,
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'    => __( 'Tag Name', 'radar' ),
			'section'  => 'featured_content',
			'priority' => 20,
		) );
		$wp_customize->add_control( 'featured-content[quantity]', array(
			'label'   => __( 'Quantity', 'radar' ),
			'section' => 'featured_content',
			'priority'=> 25,
		) );		
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'    => __( 'Don&rsquo;t display tag on front end.', 'radar' ),
			'section'  => 'featured_content',
			'type'     => 'checkbox',
			'priority' => 30,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'radar-featured-content-customize', self::$uri . '/featured-content-customize.js', array( 'jquery', 'suggest' ), '', true );
	}
